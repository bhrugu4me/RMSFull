    ...
    public class GetProductCodes
    {
        ...
        public static async Task<HttpResponseMessage> Run(HttpRequestMessage req, ICollector<string> errormessage, TraceWriter log)
        {
            ...
            try
            {
                ...
                log.Verbose($"calling sp", "JE.RMS.Services.GetProductCodes");

                List<Common.Model.ProductCodes> obj = MSSQLConnection.ExecuteStoredProcedure<Common.Model.ProductCodes>(Common.Constants.USPContstants.GetProductCodes);
                log.Verbose($"received response:={obj}", "JE.RMS.Services.GetProductCodes");
